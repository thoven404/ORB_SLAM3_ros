    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        ...
        cout << (*settings_) << endl;   // Senmentation fault (core dumped)
    }
    else{
        settings_ = nullptr;
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(settings_ && settings_->needToResize()){
        ...
        // cv::imshow("im1", resizedIm);
        // cv::waitKey(1);
        // cout << "im count: " << count << endl;
        // count++;
        imToFeed = resizedIm;
    }
    // imshow("im", im);
    ...
    // cout << "TrackMonocular bug check-3" << endl;
    ...
    // cout << "TrackMonocular bug check-4" << endl;

    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);
    // cout << "TrackMonocular bug check-5" << endl;

    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename);

    // cout << "TrackMonocular bug check-6" << endl;
